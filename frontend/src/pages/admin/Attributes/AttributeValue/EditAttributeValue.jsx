...
function EditAttributeValue({ refreshAttributeValue, name, id }) {
    ...
    async function handleUpdate(e) {
        ...
        try {
            if (uptname != name) {
                setLoading(true);
                const res = await api.patch(`attributes/updateValue/${id}`, { updatedName: uptname })
                console.log("uptname")
                if (res.status === 200) {
            ...
            <Dialog open={open} onOpenChange={setOpen} >
                ...
                <DialogContent className="sm:max-w-sm">
                    <form onSubmit={handleUpdate}>
                        <DialogHeader>
                            <DialogTitle className='font-bold'>Update Value</DialogTitle>
                        </DialogHeader>
